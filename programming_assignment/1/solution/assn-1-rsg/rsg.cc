...
static vector<string> realizeToken(string token, map<string, Definition> grammar){
    assert(grammar.count(token) > 0);
    if(token[0] != '<' || token.back() != '>')
        return vector<string>(1, token);
    vector<string> ret;
    for(auto sub_token: grammar[token].getRandomProduction()){
        vector<string> sub_ret = realizeToken(sub_token, grammar);
        ret.insert(ret.end(), sub_ret.begin(), sub_ret.end());
    }
    return ret;
}

static string joinTokens(vector<string> tokens, set<string> punctuations){
    string ret;
    for(auto it = tokens.begin(); it != tokens.end(); it++){
        ret.insert(ret.end(), (*it).begin(), (*it).end());
        if((it + 1) == tokens.end())
            continue;
        if(punctuations.count(*(it + 1)) > 0)
            continue;
        ret.push_back(' ');
    }
    return ret;
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < 3; i++){
    ...
    vector<string> realization = realizeToken("<start>", grammar);
    string sentence = joinTokens(realization, punctuations);
