...
static void DeleteEverythingVerySlowly(vector *numbers)
{
  ...
  while (VectorLength(numbers) >= 100) {
    // fprintf(stdout, "length before deletion is %d\n", VectorLength(numbers));
    VectorDelete(numbers, VectorLength(numbers) - 100);
    // fprintf(stdout, "length after deletion is %d\n", VectorLength(numbers));
    // fprintf(stdout, "%ld, %ld\n", largestOriginalNumber,
    //         *(long *)VectorNth(numbers, VectorLength(numbers) - 1));
    // fflush(stdout);
...
static void MemoryTest()
{
  ...
  for (i = 0; i < kNumQuestionWords; i++) {
    questionWord = (char *)malloc(strlen(kQuestionWords[i]) + 1);
